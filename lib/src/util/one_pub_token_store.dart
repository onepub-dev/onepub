...
class OnePubTokenStore {
  ...
  void save({required String onepubToken, required String obfuscatedPublisherId}) {
    withEnvironment(() {
      OnePubSettings().obfuscatedPublisherId = obfuscatedPublisherId;
      OnePubSettings().save();
      clearOldTokens();
      tokenStore.addCredential(Credential.token(_hostedUrl(obfuscatedPublisherId), onepubToken));
    }, environment: {onepubSecretEnvKey: onepubToken});
  }

// True if we have a onepub token.
  bool get isLoggedIn => tokenStore.findCredential(_hostedUrl(OnePubSettings().obfuscatedPublisherId)) != null;

  /// throws [StateError] if called when not logged in.
  /// returns the onepubToken.
  String fetch() {
    final credentials = tokenStore.findCredential(_hostedUrl(OnePubSettings().obfuscatedPublisherId));
  ...
  void clearOldTokens() {
    tokenStore.removeMatchingCredential(validateAndNormalizeHostedUrl(OnePubSettings().onepubApiUrl));
    // tokenStore
