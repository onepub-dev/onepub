...
Use `onepub export` to obtain the OnePub token.
  ...
  Future<void> import() async {
    ...
    try {
      ...
      if (await tokenStore.isLoggedIn(OnePubSettings.use().onepubApiUrl)) {
        initiatorOnepubToken = await tokenStore.load();
      }
    } catch (_) {
      // ignore: inability to resolve existing login should not block import.
    }
    ...
    try {
      await API().logTokenImport(
        onepubToken: onepubToken,
        initiatorOnepubToken: initiatorOnepubToken,
        tokenSource: source,
        ci: _isCi(),
        ciProvider: _ciProvider(),
        host: _firstEnv(['HOSTNAME', 'COMPUTERNAME']),
        user: _firstEnv(['USER', 'USERNAME']),
        os: Platform.operatingSystem,
        shell: _firstEnv(['SHELL']),
      );
    } catch (e, st) {
      // Logging import telemetry must never block token import.
      verbose(() => 'Unable to write import audit log: $e');
      verbose(() => '$st');
    }
...
Found: ${argResults!.rest.join(',')}''');
  ...
  String? _ciProvider() {
    ...
    for (final entry in envKeys.entries) {
      if (Env().exists(entry.key) &&
          env[entry.key] != null &&
          env[entry.key]!.isNotEmpty) {
        return entry.value;
      }
    }
    return null;
  ...
  String? _firstEnv(List<String> keys) {
    for (final key in keys) {
      if (Env().exists(key) && env[key] != null && env[key]!.isNotEmpty) {
        return env[key];
      }
    }
    return null;
